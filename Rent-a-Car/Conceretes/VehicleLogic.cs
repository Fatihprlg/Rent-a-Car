...
namespace Rent_a_Car.Conceretes
{
    public class VehicleLogic : IDisposable
    {
        ...
        public IList<Vehicle> FilterVehiclesByDate( DateTime startDate, DateTime endDate)
        {
            IList<Vehicle> vehicles = new List<Vehicle>();
            try
            {
                using (var repo = new VehicleRepository())
                {
                    vehicles = repo.FilterVehicles(startDate, endDate);
                }
                return vehicles;
            }
            catch (Exception ex)
            {
                LogHelper.Log(LogTarget.File, ExceptionHelper.ExceptionToString(ex), true);
                throw new Exception("BusinessLogic:VehicleLogic::FilterVehiclesByDate::Error occured.", ex);
            }
